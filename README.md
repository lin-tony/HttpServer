...
## 使用http_load
__测试环境__
远程访问，云主机1核，Http服务器使用了1个线程，带宽1Mbps
...
理想的测试环境是两台计算机，带宽无限，然而这种情况应该只有本地测试才能勉强达到，否则用以太网都会被限制100Mbps，所以最终选择了本地单机测试，缺点就是所有CPU要是占满了就会相互影响。
...
平均每秒钟响应请求数：1727655，每秒钟传输数据量13878611字节，
返回数：575885次返回成功，0失败。
```shell
...
__总结__
- 所担心的占满所有CPU的情况并没有发生，极限大概是180%CPU
